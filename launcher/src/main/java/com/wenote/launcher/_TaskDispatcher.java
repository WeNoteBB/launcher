...
import com.wenote.launcher.sort.TaskSortUtil;
import com.wenote.launcher.stat.TaskStat;
import com.wenote.launcher.task.DispatchRunnable;
import com.wenote.launcher.task.Task;
import com.wenote.launcher.task.TaskCallBack;
import com.wenote.launcher.utils.DispatcherLog;
import com.wenote.launcher.utils.Utils;
...
public class _TaskDispatcher {
     ...
     * @return
     */
    public static _TaskDispatcher createInstance() {
        if (!sHasInit) {
            throw new RuntimeException("must call TaskDispatcher.init first");
        }
        return new _TaskDispatcher();
    }

    public _TaskDispatcher addTask(Task task) {
        if (task != null) {
            collectDepends(task);
            mAllTasks.add(task);
